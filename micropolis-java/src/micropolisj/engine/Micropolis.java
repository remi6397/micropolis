...
public class Micropolis
{
    ...
    public void takeLoan(int amount) {
        ...
        loan.amountToPayOff = amount + additionalCharge;
        loan.monthlyPayoff = monthlyPayoff;
        budget.loans.add(loan);
    ...
    public boolean isLoanAvailable(int amount) {
        for (CityBudget.Loan loan : budget.loans) {
            if (loan.initialAmount == amount) {
                return false;
            }
        }
    ...
    void collectTax()
    {
        ...
        int totalLoanPayoff = 0;
        for (CityBudget.Loan loan : budget.loans) {
            if (loan.amountToPayOff - loan.monthlyPayoff > -loan.monthlyPayoff) {
                totalLoanPayoff += loan.monthlyPayoff;
                loan.amountToPayOff -= loan.monthlyPayoff;
            }
        }

        int expenses = -(budget.roadFundEscrow
                + budget.fireFundEscrow
                + budget.policeFundEscrow) / TAXFREQ;

        FinancialHistory hist = new FinancialHistory();
        hist.cityTime = cityTime;
        hist.taxIncome = revenue;
        hist.loanPayoff = -totalLoanPayoff;
        hist.operatingExpenses = expenses;

        cashFlow = revenue - expenses - totalLoanPayoff;
        spend(-cashFlow);
    ...
    public BudgetNumbers generateBudget()
    {
        ...
        b.newBalance = b.previousBalance + b.taxIncome - b.operatingExpenses - b.loanPayoff;
            ...
            throws IOException
    {
        ...
        // read loan data: you shall not avoid payoff!
        //
        int loanInitialAmount,
                loanPayoffLeft,
                loanMonthlyPayoff,
                i = 66; // TODO: Maybe this should be obtained from the inital amount?
        while ((loanInitialAmount = dis.readInt()) != -1) {
            loanPayoffLeft = dis.readInt();
            loanMonthlyPayoff = dis.readInt();

            CityBudget.Loan loan = new CityBudget.Loan();
            loan.initialAmount = loanInitialAmount;
            loan.amountToPayOff = loanPayoffLeft;
            loan.monthlyPayoff = loanMonthlyPayoff;
            budget.loans.add(loan);
            i += 6;
        }
            ...
            throws IOException
    {
        ...
        // write loan data: you shall not avoid payoff!
        //
        // 64
        int i = 66; // -1 offset
        for (CityBudget.Loan loan : budget.loans) {
            out.writeInt(loan.initialAmount);
            out.writeInt(loan.amountToPayOff);
            out.writeInt(loan.monthlyPayoff);
            i += 6;
        }
        out.writeInt(-1);
